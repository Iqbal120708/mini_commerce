...
class TestAddToCart(BaseTestCase):
    def test_success(self):
        ...
        self.assertIn("Produk berhasil ditambahkan ke keranjang!", [str(m) for m in msgs])
    ...
    def test_add_to_cart_validation_error(self):
        ...
        self.assertRedirects(res, reverse("product_list"))
        self.assertEqual(Cart.objects.all().count(), 0)
        self.assertIn(f"Produk '{product.name}' ini sudah habis stoknya." ,[str(m) for m in msgs])
...
class TestCartList(BaseTestCase):
    ...
    def test_cart_list_no_items_selected(self):
        ...
        res = self.client.post(reverse("cart_list"), data={"selected[]":[], "qty_1":1})
    ...
    def test_cart_list_invalid_cart_id(self):
        ...
        res = self.client.post(reverse("cart_list"), data={"selected[]":[10], "qty_10":1})
    ...
    def test_cart_list_quantity_less_than_one(self):
        ...
        res = self.client.post(reverse("cart_list"), data={"selected[]":[1], "qty_1":0})
        
        msgs = list(get_messages(res.wsgi_request))
        
        self.assertEqual(res.status_code, 302)
        self.assertRedirects(res, reverse("cart_list"))
        self.assertIn("Jumlah pembelian Sambal Matah Bali minimal 1" ,[str(m) for m in msgs])
    ...
    def test_cart_list_invalid_quantity_input(self):
        ...
        self.assertEqual(res.status_code, 302)
        self.assertRedirects(res, reverse("cart_list"))
        self.assertIn("Jumlah pembelian Sambal Matah Bali tidak valid" ,[str(m) for m in msgs])
    ...
    def test_cart_list_stock_not_enough(self):
        ...
        res = self.client.post(reverse("cart_list"), data={"selected[]":[1], "qty_1":100})
    ...
    def test_cart_list_success_order(self):
        ...
        res = self.client.post(reverse("cart_list"), data={"selected[]":[1], "qty_1":1})
